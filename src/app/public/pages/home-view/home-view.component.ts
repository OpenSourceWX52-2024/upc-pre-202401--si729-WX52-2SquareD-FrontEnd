...
export class HomeViewComponent implements OnInit {
  ...
  initializeCarousel() {
    ...
    CARD_CONTAINER.addEventListener('scroll', () => {
      this.currentIndex = Math.round(CARD_CONTAINER.scrollLeft / CARD_CONTAINER.clientWidth);
    });
    ...
    const ITEMS = CARD_CONTAINER && CARD_CONTAINER.children ? Array.from(CARD_CONTAINER.children) : [];
    ...
    const SET_SCROLL_BOUNDS = () => {
      (ITEMS[ITEMS.length - 1] as HTMLElement).scrollIntoView();
      scrollBounds.max = CARD_CONTAINER.scrollLeft + (ITEMS[0] as HTMLElement).offsetWidth;
      (ITEMS[0] as HTMLElement).scrollIntoView();
      scrollBounds.min = CARD_CONTAINER.scrollLeft - (ITEMS[0] as HTMLElement).offsetWidth;
    };
